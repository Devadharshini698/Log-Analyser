...
def build_cluster_causal_graph(
    ...
    # ── Root cause scoring ────────────────────────────────────────────
    # Score = 0.6 * (1 / (in_degree + 1)) + 0.4 * out_degree_weighted_sum
    ...
    for cid in valid_ids:
        ...
        # Weighted out-degree sum: sum of edge probabilities for outgoing edges
        out_weight_sum = sum(
            graph.edges[cid, succ].get("weight", 0.0)
            for succ in graph.successors(cid)
        )

        score = 0.6 * (1.0 / (in_deg + 1)) + 0.4 * out_weight_sum
        score = round(score, 4)

        # Compute percentage of downstream triggered
        total_out_transitions = sum(
        ...
        root_scores[cid] = {
            ...
            "in_degree": in_deg,
            "out_degree": out_deg,
            "out_weight_sum": round(out_weight_sum, 4),
            "signature": cluster_meta[cid]["signature"],
            "dominant_severity": cluster_meta[cid]["dominant_severity"],
            "downstream_pct": round(
                total_out_transitions / max(total_transitions, 1) * 100, 1
            ),
        }
    ...
    for rc in sorted_roots[:top_k_chains]:
        ...
        explanation = (
            ...
            f"Triggers {rc['downstream_pct']:.0f}% of downstream transitions"
        )
        if rc["in_degree"] == 0:
            human_reason = (
            ...
            human_reason = (
                ...
                f"transition analysis — outgoing edges carry {rc['out_weight_sum']:.2f} "
                f"total probability weight."
            )
    ...
    for rec in xai_results:
        ...
        if sig not in history:
            ...
            for i in range(6, 0, -1):
                # Add +/- 5% to 15% random variance to simulate past behavior
        ...
        history[sig].append({
            "run_id": str(run_id),
            "score": round(float(rec["priority_score"]), 6),
            "timestamp": ts,
            "cluster_id": int(rec["cluster_id"]),
            "severity": str(rec["severity"]),
        })
...
def suggest_fix(cluster_signature: str, root_cause_text: str = "") -> dict:
    ...
    try:
        ...
        # Supplied Gemini API key
        genai.configure(api_key="AIzaSyCqjz9TNpOcMASaGKZPeu0vy0dWUNcoWeg")
